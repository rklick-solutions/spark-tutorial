    ...
    /*def getEdges(df1: DataFrame): RDD[Edge[String]] = {
      df1.map {
        ...
        def getEdges(df1: DataFrame): RDD[Edge[String]] = {
          df1.map {
            case row => Edge(row.getAs[Any]("id").toString.toLong,
              row.getAs[Any]("id").toString.toLong, row.getAs[Any]("location").toString)
          }
        }
...
/*
   ...
   // graph.edges.distinct.collect.foreach(println)
   // graph.vertices.distinct.collect.foreach(println)
